...
inputData
summary(inputData)
sapply(inputData, sd)   #if we want SD for each and every column
str(inputData)
...
xtabs(~admit + rank, data = data)   #xtab: similar to pivot table of excel

#Create Logistic Model
mylogit <- glm(admit ~ gre + gpa + rank, data = data, family = "binomial")  # = and <- is same

summary(mylogit)    #AIC:lesser the better
...
#cutoff value
library(InformationValue)   #helps in finding optimal cutoff
...
(newdata1 = data.frame(gre=450, gpa=3.7, rank=factor(3) ))  #rank converted to factor bcz we converted rank into factor in line 15; datatype shuld be same
