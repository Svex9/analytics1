...
fit1      #y dependent and x independent-->value of y fit around the best fit line;remaining are residual
summary(fit1)
...
#predict for area = 4
#using equation
(Y = coef(fit1)[1] + coef(fit1)[2] * 4)   #[1] showing the value at 1st position
#using model
...
#RMSE(root mean square error) is generally used and should be least when selecting models.lesser the better
...
plot(fit1)   #it will show all the 4 plots
par(mfrow=c(2,2))
...
#Auto Collinearity : relation between successive values of Y:today's sales dependent upon yesterday's sales
...
residuals(fit1)
plot(fit1, which=2)
#points to be around the straight line means residual is normally distributed
...
#if cook's distance vaue is >0.5 then that value is an outlier:needs to be removed
