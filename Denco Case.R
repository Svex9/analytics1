...
str(sales)     #shows structure of data frame
class(sales)
str(sales)
summary(sales)
...
library(dplyr)
sales %>% dplyr::count(custname, sort=TRUE)     #to find top customers
...
sales %>% group_by(region,custname) %>% summarise(TotalRevenue=min(revenue)) %>% arrange(desc(TotalRevenue))  %>% top_n(n=2)

sales%>%group_by(region,custname)%>%summarise(maxRevenue=max(revenue))%>%arrange(desc(region))%>%arrange(desc(custname))%>%top_n(n=2)
...
#Reqmt-8 : create another column with 10% decrease in column margin
